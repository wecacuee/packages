...
KVER=4.15
aero-kernel/linux_$(KVER).0.orig.tar.gz:
	cd aero-kernel && wget http://archive.ubuntu.com/ubuntu/pool/main/l/linux/linux_$(KVER).0.orig.tar.gz

aero-kernel/linux_$(KVER).0-49.53.diff.gz:
	cd aero-kernel && wget http://archive.ubuntu.com/ubuntu/pool/main/l/linux/linux_$(KVER).0-49.53.diff.gz

kernel-source: aero-kernel/linux_$(KVER).0.orig.tar.gz aero-kernel/linux_$(KVER).0-49.53.diff.gz
	cd aero-kernel && tar -xf linux_$(KVER).0.orig.tar.gz
	cd aero-kernel && gunzip linux_$(KVER).0-49.53.diff.gz
	cd aero-kernel/linux-$(KVER) && patch -p1 < ../linux_$(KVER).0-49.53.diff

kernel-patch: kernel-source
	cd aero-kernel/linux-$(KVER) && for i in `find ../patches-4.4/ -name "*.patch" | sort` ; \
			do echo "applying patch $$i" && patch -p1 < $$i ; done
	touch aero-kernel/linux-$(KVER)/.scmversion
	cp aero-kernel/patches/defconfig aero-kernel/linux-$(KVER)/.config
	cd aero-kernel/linux-$(KVER) && yes '' | make oldconfig

kernel: kernel-patch
	cd aero-kernel/linux-$(KVER) && make deb-pkg -j5
